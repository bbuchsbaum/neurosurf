...
3.  **Check Beads state after changes** with `bd vc status`; this
    checkout uses the embedded Dolt backend with auto-commit enabled, so
    the older manual sync subcommand is not available. If pending Beads
    changes are reported, run `bd dolt commit -m "message"`.
