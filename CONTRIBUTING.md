...
Both build targets work from a clean `git archive` of the source commit with
the runtime patch applied, so uncommitted changes in the checkout never reach
the bundle. `make surfview` installs nothing unless the rebuilt bundle matches
the recorded SHA-256. To change the runtime patch, edit
`tools/surfview-runtime.patch` and run `make surfview-repin`; commit the
bundle, marker, and patch together.
