...
by [surfviewjs](https://github.com/bbuchsbaum/surfviewjs). The report-safe
embed bundle is committed, so package users do not need Node. Maintainers
control the source checkout at `~/code/jscode/surfviewjs`; `make
surfview` rebuilds it and records the exact source commit and artifact
checksum in
...
defaults. It is not a widget class or an interaction mode. `mode =
"report"` separately enables the compact map, anatomical-view, reset,
fullscreen, and PNG controls. The old Tweakpane control surface is
deprecated and is not included in generated reports.
