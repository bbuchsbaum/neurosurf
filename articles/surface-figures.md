...
anatomical context, use sulcal depth or curvature from the same subject
or template rather than curvature computed on the inflated display
shape. Sulci render darker than gyri; the renderer detects whether a
metric is larger in sulci (as FreeSurfer `sulc` and `curv` are) and
orients it accordingly.

The package bundles the fsaverage5 template (10,242 vertices per
hemisphere) with its sulcal depth, so this example runs offline.

``` r

inflated <- load_fsaverage("fsaverage5", "inflated")
white <- load_fsaverage("fsaverage5", "white")
sulc <- load_fsaverage_sulc("fsaverage5")
```

Here smooth clusters placed in white-surface coordinates stand in for
your measured statistic. The renderer treats them exactly like any other
vertex-wise numeric vector.

``` r

example_statistic <- function(geometry, side) {
  cluster <- function(centre, z, width = 12) {
    z * exp(-colSums((t(coords(geometry)) - centre)^2) / (2 * width^2))
  }
  cluster(c(45 * side, -60, 20), 4) + cluster(c(40 * side, 20, 30), 3) +
    cluster(c(55 * side, -20, 0), -3.5) + cluster(c(8 * side, 40, 10), -3)
}

stat_lh <- example_statistic(white$lh, -1)
stat_rh <- example_statistic(white$rh, 1)
```

## How do you render one thresholded view?

Choose the threshold and colour limits as scientific inputs. In this
example, the colour scale stays fixed at `[-4, 4]`, while an opacity
ramp softens only the visual transition immediately above `|z| = 1.5`.
...
Surfaces are lit with interpolated normals, so cortical form reads
without drawn outlines, and the overlay keeps its colour-bar colour to
within a few percent at any viewing angle. Coarse meshes are subdivided
for display (`subdivide = "auto"`); the subdivision keeps every original
vertex value and never creates a suprathreshold region that linear
