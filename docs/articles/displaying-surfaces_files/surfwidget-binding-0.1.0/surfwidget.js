    ...
    return {
        ...
        try {
          ...
          if (x.cmap) {
            ...
            if (Array.isArray(x.layers) && x.layers.length > 0 && neurosurface.MultiLayerNeuroSurface) {
              neurosurface.debugLog('Creating MultiLayerNeuroSurface');
          ...
          drawColorbar(
            barCmap,
            barRange,
            x.colorbar ? x.colorbar.label : null,
            viewerHeight,
            colorbarEnabled
          );
      ...
      resize: function(width, height) {
        if (viewer) {
          ...
          const primaryLayer = (lp.layers || []).find(l => !l.as_outline);
          const barCmap = lp.cmap || (primaryLayer && primaryLayer.cmap);
          const barRange = lp.irange || (primaryLayer && (primaryLayer.color_range || primaryLayer.irange));
          drawColorbar(barCmap, barRange, lp.colorbar ? lp.colorbar.label : null, viewerHeight, colorbarEnabled);
        }
      },
