...
object with the smoothed values. The projection is performed by finding
the closest points on the surface, and then a kernel density smoother is
applied locally to produce the final values.
...
- surfgeom:
  ...
  object representing the surface onto which the coordinates will be
  projected.
