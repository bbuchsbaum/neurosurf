...
- panels:
  ...
  and its subclasses, or a `SurfaceSet`), a file path readable by
  [`read_surf_geometry`](https://bbuchsbaum.github.io/neurosurf/reference/read_surf_geometry.md),
  or a list whose first *unnamed* element is such an object and whose
  remaining *named* elements are per-panel display arguments (for
  example `thresh`, `vals`, `cmap`, `irange`, or `viewpoint`). Per-panel
  arguments override the shared arguments passed through `...`.
...
- ...:
  ...
  (for `SurfaceGeometry` panels) or to the object's `plot` method (for
  data surfaces).
