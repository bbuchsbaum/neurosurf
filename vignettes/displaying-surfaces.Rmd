...
if (requireNamespace("Rvcg", quietly = TRUE)) {
  try({
    ...
    if (is.function(Rvcg::vcgSubdivide)) {
      white_lh_hi@mesh <- Rvcg::vcgSubdivide(white_lh_hi@mesh, type = "loop", iterations = 2, silent = TRUE)
    }
    white_lh_hi <- smooth(white_lh_hi, type = "HCLaplace", delta = 0.15, iteration = 8)
...
# Recompute curvature for the current display mesh and smooth it repeatedly
# to remove the lattice artifact that subdivision introduces
curv_lh_display <- tryCatch({
  raw_curv <- curvature(white_lh_display)
  curv_surf <- NeuroSurface(white_lh_display,
                            indices = seq_len(length(raw_curv)),
                            data = raw_curv)
  # Multiple smoothing passes to fully diffuse the subdivision lattice
  for (i in seq_len(3)) curv_surf <- smooth(curv_surf)
  values(curv_surf)
}, error = function(e) curvature(white_lh_display))
```
...
```{r two-hemispheres-plot, results='asis', out.width="50%", fig.show='hold'}
...
# RH lateral
render_surface(white_rh_smooth, bgcol = curv_cols_smooth(curv_rh),
               viewpoint = "lateral", specular = "black")
```
