...
- add a colourbar to produce a surfplot-style figure.
...
two views (`lateral` and `medial`), adding a continuous map (here, the
network index derived from the parcel id).
...
```{r render-figure, fig.width=8, fig.height=4, fig.show='hide', results='asis'}
...
cat(sprintf(
  "![Lateral and medial views of left and right fsaverage surfaces with network colours](%s)\n",
  fig_file
))
```
...
- a continuous network index map with a clean colourbar.
